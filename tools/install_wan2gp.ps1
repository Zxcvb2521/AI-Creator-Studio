...
# Keep the whole development runtime inside the Studio tree by default.
...
if ($isLegacy) {
  ...
  if (-not (Test-Path (Join-Path $PythonEnv "python.exe"))) { conda create -p $PythonEnv python=3.10.9 -y }
  conda run -p $PythonEnv python -m pip install torch==2.7.1 torchvision==0.22.1 torchaudio==2.7.1 --index-url https://download.pytorch.org/whl/test/cu128
} else {
  Write-Host "Using current Wan2GP environment (Python 3.11.14 / PyTorch 2.10 / CUDA 13.0)." -ForegroundColor Green
  if (-not (Test-Path (Join-Path $PythonEnv "python.exe"))) { conda create -p $PythonEnv python=3.11.14 -y }
  conda run -p $PythonEnv python -m pip install torch==2.10.0 torchvision==0.25.0 torchaudio==2.10.0 --index-url https://download.pytorch.org/whl/cu130
}

conda run -p $PythonEnv python -m pip install -r (Join-Path $InstallRoot "requirements.txt")

$envFile = Join-Path $InstallRoot "ai-creator-studio.env.json"
@{
  wan2gp_root = $InstallRoot
  python_root = $PythonEnv
  executable = "wgp.py"
} | ConvertTo-Json | Set-Content -Encoding UTF8 $envFile

Write-Host "Wan2GP installation completed." -ForegroundColor Green
Write-Host "Python: $PythonEnv\python.exe"
Write-Host "Root: $InstallRoot"
Write-Host "Test: conda run -p $PythonEnv python $InstallRoot\wgp.py"
