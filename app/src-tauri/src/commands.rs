...
use crate::{assets, capabilities, engine_dir, hardware, job_manager, jobs, model_catalog, project, resolve_python, run_adapter, runtime, runtime_dir, system_check, bridge_script};

#[tauri::command]
pub fn engine_status() -> crate::EngineStatus { let engine = engine_dir(); let runtime_dir = runtime_dir(); crate::EngineStatus { running: bridge_script().exists() && engine.join("wgp.py").exists(), runtime_dir: runtime_dir.to_string_lossy().into_owned(), engine_dir: engine.to_string_lossy().into_owned() } }
...
#[tauri::command] pub fn start_engine() -> Result<String, String> { let _ = run_adapter(&["models".into()])?; Ok(format!("Wan2GP adapter is ready · Python: {}", resolve_python().display())) }
...
#[tauri::command] pub fn stop_engine() -> Result<String, String> { Ok("Wan2GP adapter is invoked per operation; no persistent process to stop".into()) }
#[tauri::command] pub fn startup() -> crate::startup::StartupReport { crate::startup::run() }
#[tauri::command] pub fn runtime_status() -> runtime::RuntimeStatus { runtime::status() }
#[tauri::command] pub fn runtime_install() -> Result<runtime::RuntimeStatus, String> { runtime::install() }
