...
#[tauri::command] pub fn engine_status() -> EngineStatus { let engine = engine_dir(); let runtime = runtime_dir(); EngineStatus { running: bridge_script().exists() && engine.exists(), runtime_dir: runtime.to_string_lossy().into_owned(), engine_dir: engine.to_string_lossy().into_owned() } }
...
pub fn run() { tauri::Builder::default().plugin(tauri_plugin_shell::init()).invoke_handler(tauri::generate_handler![engine_status, hardware_info, system_check, capabilities, model_catalog, model_schema, startup, start_engine, generate, generation_status, cancel_generation, record_generation_assets, asset_catalog, project_load, project_save, stop_engine]).run(tauri::generate_context!()).expect("error while running AI Creator Studio"); }
