...
pub fn discover() -> ModelCatalog {
    ...
    if !root.exists() { return ModelCatalog { models: vec![], source: "wan2gp-api".into(), error: Some(format!("WanGP root not found: {}", root.display())) }; }
    if !script.exists() { return ModelCatalog { models: vec![], source: "wan2gp-api".into(), error: Some(format!("WanGP adapter not found: {}", script.display())) }; }
    let output = match Command::new("python").arg(&script).arg("--root").arg(&root).arg("models").output() { Ok(v) => v, Err(e) => return ModelCatalog { models: vec![], source: "wan2gp-api".into(), error: Some(format!("Failed to invoke WanGP adapter: {e}")) } };
