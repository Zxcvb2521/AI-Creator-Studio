...
pub fn discover() -> ModelCatalog {
    let root = engine_root(); let script = adapter_script();
    if !root.exists() { return ModelCatalog { models: vec![], source: "wan2gp-api".into(), error: Some(format!("Wan2GP root not found: {}", root.display())) }; }
    if !script.exists() { return ModelCatalog { models: vec![], source: "wan2gp-api".into(), error: Some(format!("Wan2GP adapter not found: {}", script.display())) }; }
    let output = match Command::new("python").arg(&script).arg("--root").arg(&root).arg("models").output() { Ok(v) => v, Err(e) => return ModelCatalog { models: vec![], source: "wan2gp-api".into(), error: Some(format!("Failed to invoke Wan2GP adapter: {e}")) } };
    if !output.status.success() { return ModelCatalog { models: vec![], source: "wan2gp-api".into(), error: Some(String::from_utf8_lossy(&output.stderr).trim().to_string()) }; }
    let raw: serde_json::Value = match serde_json::from_slice(&output.stdout) { Ok(v) => v, Err(e) => return ModelCatalog { models: vec![], source: "wan2gp-api".into(), error: Some(format!("Invalid model catalog: {e}")) } };
