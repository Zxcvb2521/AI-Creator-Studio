...
const ENGINE_URL: &str = "https://github.com/deepbeepmeep/Wan2GP";
...
pub fn is_ready(root: &Path) -> bool {
    ...
    let python = engine.join("env_uv").join("Scripts").join("python.exe");
    fs::read(path(root))
        .ok()
        .and_then(|bytes| serde_json::from_slice::<RuntimeManifest>(&bytes).ok())
        .map(|m| m.installed && engine.join("wgp.py").exists() && python.exists())
        .unwrap_or(false)
}
