...
pub fn load(runtime_dir: PathBuf) -> Result<ProjectState, String> {
    let path = project_path(&runtime_dir);
    if !path.exists() { return Ok(ProjectState { version: 1, ..Default::default() }); }
    serde_json::from_slice(&fs::read(path).map_err(|e| format!("Failed to read project: {e}")))
