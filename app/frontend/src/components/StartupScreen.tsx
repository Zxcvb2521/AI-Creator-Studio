...
export function StartupScreen({ onReady }: { onReady: () => void }) {
  ...
  return <main className="startup-screen"><div className="startup-card"><div className="eyebrow">AI CREATOR STUDIO</div><h1>Preparing your workspace</h1><p className="startup-subtitle">Проверяем систему и подключаем Wan2GP.</p><div className="startup-steps">{report.steps.map(step => <div className="startup-step" key={step.id}><span className={`check ${step.status}`}>{step.status === 'ready' ? '✓' : step.status === 'missing' || step.status === 'failed' || step.status === 'blocked' ? '!' : '…'}</span><div><strong>{labels[step.id] ?? step.id}</strong><small>{step.detail}</small></div></div>)}</div>{!report.steps.length && <div className="startup-loading">Starting checks…</div>}{error && <div className="error">{error}</div>}{missing.length > 0 && <div className="startup-note"><strong>Wan2GP пока не установлен.</strong><small>Это нормально для первого запуска. Установщик будет использовать F:\XTTS\Wan2GP и отдельное окружение Conda.</small></div>}{!report.ready && missing.length === 0 && <div className="startup-note"><strong>Wan2GP engine ещё не подключён.</strong><small>После установки Studio автоматически сможет использовать его как внешний engine.</small></div>}</div></main>;
}
